...
process.on('uncaughtException', err => {
    if (!err.message) {
        err = new Error(lodash_1.toString(err));
    }
    process.send({
        cmd: 'failed',
        value: err,
...
function wrapJob(job) {
    ...
    return Object.assign(Object.assign({}, job), { data: JSON.parse(job.data || '{}'), opts: JSON.parse(job.opts || '{}'), returnValue: JSON.parse(job.returnvalue || '{}'), 
        /*
